...
class MTDevice(object):
    ...
    def write_msg(self, mid, data=[]):
        ...
        if verbose:
            print
            "MT: Write message id 0x%02X (%s) with %d data bytes: [%s]" % (mid, getMIDName(mid), length,
    ...
    def read_msg(self):
        ...
        while (time.time() - start) < self.timeout:
            ...
            if verbose:
                print
                "MT: Got message id 0x%02X (%s) with %d data bytes: [%s]" % (mid, getMIDName(mid), length,
                                                                             ' '.join("%02X" % v for v in data))
    ...
    def SetCurrentScenario(self, scenario_id):
        ...
        if validateSc.any():
            ...
            print
            "Set to scenario:%2d" % scenario_id
        else:
    ...
    def configureMti(self, mtiSampleRate, mtiMode):
        ...
        print
        "Timeout changed to %1.3fs based on current settings." % (self.timeout)
        ...
        if (deviceIDProductMask[2] == XDIProductMask.MTi1Series) | (
                deviceIDProductMask[2] == XDIProductMask.FMT1000Series):
        ...
        if (deviceIDProductMask[2] == XDIProductMask.MTi100Series) & (deviceTypeMask[2] == XDIProductMask.MTi700Device):
            ...
            if mtiMode == 1:
                print
                "Enabled publishing all sensor data"
                data = mStf + mImuDq + mImuDv + mImuMag + mImuP + mGnssPvt + mGnssSat + mSw + mOrientationQuat
            elif mtiMode == 2:
                print
                "Enabled publishing all sensor data (rate quantities)"
            ...
            print
            "MTi-100/200/300 device detected."
            if mtiMode == 1:
                print
                "Enabled publishing all sensor data"
            ...
            print
            "FMT-1010/1020/1030 device detected"
            if mtiMode == 1:
                print
                "Enabled publishing all sensor data"
                data = mStf + mImuDq + mImuDv + mImuMag + mSw + mOrientationQuat
        ...
        print
        "Device configured at %1.0f bps" % (brSettings)
    ...
    def auto_config(self):
        ...
        if Config.any():
            ...
            print
            "Timeout defaults to %1.3fs based on output settings." % (self.timeout)
        else:
    ...
    def parse_MTData2(self, data):
        ...
        def parse_GNSS(data_id, content, ffmt):
            ...
            if (data_id & 0x00F0) == 0x10:  # GNSS PVT DATA
                o['iTOW'], x1, x2, x3, x4, x5, x6, x7, x8, x9, o['fix'], o['flag'], o[
                    'nSat'], x10, lon, lat, h, a, hAcc, \
                ...
                o['VDOP'], o['HDOP'], o['NDOP'], o['EDOP'], o['heading'], o[
                    'headingAcc'] = 1e-7 * lat, 1e-7 * lon, 1e-3 * h, \
    ...
    def ChangeBaudrate(self, baudrate):
        ...
        if bridAck:  # Testing if the BR was set correctly
            ...
            print
            "Baudrate set to %d bps" % baudrate
            time.sleep(0.01)
        else:
            print
            "NOK:Baudrate not configured."
...
def main():
    ...
    for o, a in opts:
        ...
        if o in ('-f', '--mti-odr'):
            try:
                ...
                print
                "MTi sample rate argument must be integer."
                return 1
        if o in ('-m', '--mti-mode'):
            try:
                mode = int(a)
                ...
                print
                "MTi mode argument must be integer."
                return 1
    ...
    try:
        ...
        if 'inspect' in actions:
            mt.GoToConfig()
            print
            "Device: %s at %d bps:" % (device, baudrate)
            print
            "General configuration:", mt.ReqConfiguration()
            print
            "Available scenarios:", mt.ReqAvailableScenarios()
            print
            "Current scenario: %s (id: %d)" % mt.ReqCurrentScenario()[::-1]
            mt.GoToMeasurement()
        if 'change-baudrate' in actions:
            print
            "Changing baudrate from %d to %d bps\n" % (baudrate, new_baudrate),
            sys.stdout.flush()
        ...
        if 'reset' in actions:
            ...
            print
            " Ok"  # should we test it was actually ok?
        if 'xkf-scenario' in actions:
            print
            "Changing XKF scenario..."
            sys.stdout.flush()
            mt.GoToConfig()
        ...
        if 'setMtiOutputConfiguration' in actions:
            ...
            mt.GoToConfig()
            print
            "Device intiated at %d Hz" % (sampleRate)
            mt.configureMti(sampleRate, mode)
        if 'echo' in actions:
            try:
                while True:
                    print
                    mt.read_measurement(mode, settings)
